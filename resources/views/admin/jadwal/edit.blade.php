                    ...
                    <form action="{{ route('admin.jadwal.update', $jadwal->id) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="mapel_id" id="mapel_id"
                                ...
                                @foreach ($mataPelajaran as $mp)
                                    <option value="{{ $mp->id }}"
                                        {{ old('mapel_id', $jadwal->mapel_id) == $mp->id ? 'selected' : '' }}>
                                        {{ $mp->nama_mapel }}</option>
                                @endforeach
                            </select>
                            @error('mapel_id')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="hari" id="hari"
                                ...
                                <option value="Senin" {{ old('hari', $jadwal->hari) == 'Senin' ? 'selected' : '' }}>
                                    Senin</option>
                                <option value="Selasa" {{ old('hari', $jadwal->hari) == 'Selasa' ? 'selected' : '' }}>
                                    Selasa</option>
                                <option value="Rabu" {{ old('hari', $jadwal->hari) == 'Rabu' ? 'selected' : '' }}>
                                    Rabu</option>
                                <option value="Kamis" {{ old('hari', $jadwal->hari) == 'Kamis' ? 'selected' : '' }}>
                                    Kamis</option>
                                <option value="Jumat" {{ old('hari', $jadwal->hari) == 'Jumat' ? 'selected' : '' }}>
                                    Jumat</option>
                                <option value="Sabtu" {{ old('hari', $jadwal->hari) == 'Sabtu' ? 'selected' : '' }}>
                                    Sabtu</option>
                        ...
                        <div class="flex items-center justify-end mt-4">
                            ...
                            <x-primary-button type="submit">Update Jadwal</x-primary-button>
                        </div>
                    </form>
                </div>
            </div>
